...
Deno.test("NIP-33 store - parameterized replaceable replaces by kind+pubkey+d-tag", () => {
...
Deno.test("NIP-33 store - different d-tags are separate events", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
...
Deno.test("NIP-33 handleEvent - parameterized replaceable via client EVENT", () => {
  const pool = new MockPool();
...
Deno.test("NIP-33 - #d tag filter works with parameterized replaceable", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
...
Deno.test("NIP-33 - different pubkey with same d-tag are separate", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
