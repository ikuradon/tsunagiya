...
Deno.test("matchFilter - ids prefix match", () => {
...
Deno.test("matchFilter - authors prefix match", () => {
...
Deno.test("matchFilter - kinds exact match", () => {
...
Deno.test("matchFilter - since/until range", () => {
...
Deno.test("matchFilter - tag filters", () => {
...
Deno.test("matchFilter - combined conditions (AND)", () => {
  const event = makeEvent({
    id: "abc123",
    pubkey: "pub456",
    kind: 1,
...
Deno.test("matchFilter - empty filter matches everything", () => {
  const event = makeEvent();
  assertEquals(matchFilter(event, {}), true);
});

Deno.test("matchFilters - OR across filters", () => {
  const event = makeEvent({ kind: 1 });
...
Deno.test("filterEvents - sorts by created_at descending", () => {
  const events = [
...
Deno.test("filterEvents - filters out non-matching", () => {
  const events = [
