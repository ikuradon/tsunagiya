...
Deno.test("NIP-16 store - replaceable event replaces same kind+pubkey", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
...
Deno.test("NIP-16 store - ephemeral event broadcasts to subscriptions", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
...
Deno.test("NIP-16 store - different pubkey replaceable events coexist", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
...
Deno.test("NIP-16 handleEvent - ephemeral returns OK but not stored", () => {
  const pool = new MockPool();
...
Deno.test("NIP-16 handleEvent - replaceable replaces via client EVENT", () => {
  const pool = new MockPool();
  pool.relay("wss://relay.example.com");
...
Deno.test("NIP-16 handleEvent - onEVENT handler skips auto-processing", () => {
  const pool = new MockPool();
  const relay = pool.relay("wss://relay.example.com");
