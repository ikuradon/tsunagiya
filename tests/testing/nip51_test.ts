...
Deno.test("EventBuilder.muteList() - adds e and word tags", () => {
  const event = EventBuilder.muteList({
    eventIds: ["event1"],
    words: ["spam", "scam"],
  }).build();
...
Deno.test("EventBuilder.muteList() - supports all tag types", () => {
  const event = EventBuilder.muteList({
    pubkeys: ["pub1"],
    eventIds: ["event1"],
...
Deno.test("EventBuilder.muteList() - produces no tags with empty options", () => {
...
Deno.test("EventBuilder.bookmarks() - adds t tags for hashtags", () => {
  const event = EventBuilder.bookmarks({
